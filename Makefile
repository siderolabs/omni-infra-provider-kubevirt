...
all: unit-tests omni-infra-provider-kubevirt image-omni-infra-provider-kubevirt docker-compose-up docker-compose-down lint
...
image-omni-infra-provider-kubevirt:  ## Builds image for omni-infra-provider-kubevirt.
	@$(MAKE) target-$@ TARGET_ARGS="--tag=$(REGISTRY)/$(USERNAME)/omni-infra-provider-kubevirt:$(IMAGE_TAG)"
