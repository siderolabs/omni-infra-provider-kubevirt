...
.PHONY: sign-image-omni-infra-provider-kubevirt
sign-image-omni-infra-provider-kubevirt:  ## Signs the image for omni-infra-provider-kubevirt. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/omni-infra-provider-kubevirt:$(IMAGE_TAG)
