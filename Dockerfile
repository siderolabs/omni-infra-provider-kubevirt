...
#
# Generated on 2026-05-28T08:53:48Z by kres 3236602.
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-kubevirt/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-kubevirt/go/pkg go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /omni-infra-provider-kubevirt-linux-amd64
