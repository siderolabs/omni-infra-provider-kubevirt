...
#
# Generated on 2026-07-23T13:28:02Z by kres 5f4a4c6.
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=omni-infra-provider-kubevirt/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=omni-infra-provider-kubevirt/go/pkg go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=omni-infra-provider-kubevirt -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /omni-infra-provider-kubevirt-linux-amd64
